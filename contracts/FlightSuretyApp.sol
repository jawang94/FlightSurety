...
contract FlightSuretyApp {
    ...
    FlightSuretyDataInterface flightSuretyData; // The data contract state variable

    /********************************************************************************************/
    /*                                       EMITTING METHODS                                   */
    /********************************************************************************************/

    event AirlineRegistered(bool status, string info, uint256 votes);
    event FlightRegistered(uint8 status, string info, bytes32 key);
    event InsuranceClaimed(address customer, uint256 amount);
    ...
    modifier requireIsAnActiveAirline(address airline) {
        ...
        require(
            this.isActiveAndFunded(airline),
            "Caller is not yet an active airline"
        );
        _;
    }
    ...
    function getAirlineByAddress(address airlineAddress)
        external
        view
        returns (
            string memory,
    ...
    function calculateInsuranceAmount() internal returns (uint256 amount) {
        amount = msg.value;
    ...
    {
        (bool success, string memory message) = flightSuretyData
        ...
        if (this.isActive(airline)) {
            emit AirlineRegistered(
                true,
                "Airline accepted, waiting for funding",
                0
            );
        }
    }
    ...
    {
        (bool success, string memory message) = flightSuretyData.registerFlight(
            msg.sender,
            flight,
            timestamp,
            tickets
     ...
     */
    function buyInsurance(string calldata flight, uint256 ticket)
        external
        payable
        requireIsOperational
        checkForRefund(calculateInsuranceAmount())
    {
        flightSuretyData.buy.value(calculateInsuranceAmount())(
            flight,
            ticket,
            msg.sender
        );
     ...
     */
    function receivePayout() external payable requireIsOperational {
...
contract FlightSuretyDataInterface {
    ...
    function getAirlineTotal() external view returns (uint256);
    ...
    function pay(address payable receiver)
        external
        payable
