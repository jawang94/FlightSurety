...
contract FlightSuretyData {
     ...
     */
    function getAirlineByName(string calldata name)
        external
        view
        returns (address)
    {
        ...
        for (uint256 i = 0; i < airlinesList.length; i++) {
            if (
                keccak256(bytes(airlinesList[i].name)) == keccak256(bytes(name))
            ) {
    ...
    function getFlightKeyByName(string memory flight)
        internal
        view
        returns (bytes32 key)
    {
        ...
        for (uint8 i = 0; i < flightsList.length; i++) {
            if (
                keccak256(bytes(flight)) ==
                keccak256(bytes(flightsList[i].name))
            ) {
     ...
     */
    function registerFirstAirline(address airline) internal {
    ...
    function fund(address airline) public payable {
        ...
        airlinesList.push(
            AirlineList({name: airlines[airline].name, addr: airline})
        );
    }
    ...
    function registerFlight(
        ...
        flightsList.push(FlightList({name: flight, key: key}));
    ...
    function getAllFlightsName() external view returns (bytes32[] memory) {
        ...
        for (uint256 i = 0; i < flightsList.length; i++) {
